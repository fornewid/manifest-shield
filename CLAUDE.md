...
- **`true` by default** (security/capability): `usesPermission`, `usesFeature`, `activity`, `activityAlias`, `service`, `receiver`, `provider`, `startup`, `queries`
- **`false` by default** (verbose/niche): `usesSdk`, `permission`, `intentFilter`, `usesPermissionSdk23`, `supportsScreens`, `compatibleScreens`, `usesConfiguration`, `supportsGlTexture`, `metaData`, `usesLibrary`, `usesNativeLibrary`, `profileable`
